...
const Signup = () => {
  ...
  const handleSignup = async (e) => {
    ...
    if (!hasLength || !hasUpper || !hasLower) {
      Swal.fire({
        icon: "error",
        title: "Weak Password",
        text: "Please satisfy all password criteria listed below.",
  ...
  return (
    <div className="min-h-screen flex justify-center items-center bg-gradient-to-tr from-slate-900 via-slate-850 to-amber-950 px-4 py-12 relative overflow-hidden">
      ...
      {/* Decorative blurred soft light blobs */}
      ...
      <div className="w-full max-w-lg bg-white/95 backdrop-blur-md p-8 md:p-10 rounded-3xl shadow-2xl border border-white/20 relative z-10 transition-all duration-300">
        ...
        <form onSubmit={handleSignup} className="space-y-5">
          ...
          { }
          {password && (
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-2 text-xs transition-all duration-300">
              <p className="text-slate-500 font-bold mb-1">Password Requirements:</p>
              <div className="flex items-center gap-2">
                <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] text-white font-bold transition-all duration-300 ${hasLength ? 'bg-emerald-500' : 'bg-slate-300'}`}>✓</span>
                <span className={hasLength ? 'text-emerald-600 font-semibold' : 'text-slate-400'}>Minimum 6 characters</span>
              </div>
              <div className="flex items-center gap-2">
                <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] text-white font-bold transition-all duration-300 ${hasUpper ? 'bg-emerald-500' : 'bg-slate-300'}`}>✓</span>
                <span className={hasUpper ? 'text-emerald-600 font-semibold' : 'text-slate-400'}>At least one uppercase letter (A-Z)</span>
              </div>
              <div className="flex items-center gap-2">
                <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] text-white font-bold transition-all duration-300 ${hasLower ? 'bg-emerald-500' : 'bg-slate-300'}`}>✓</span>
                <span className={hasLower ? 'text-emerald-600 font-semibold' : 'text-slate-400'}>At least one lowercase letter (a-z)</span>
              </div>
            </div>
          )}
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-white px-3 text-slate-400 font-semibold tracking-wider">Or Register With</span>
          </div>
        </div>
